...
import { Text, View, StyleSheet, FlatList, Dimensions, Image, TouchableOpacity, Platform } from 'react-native'
                ...
                <View style={{width:'100%', height:70, padding:10, justifyContent:'center', position:'absolute', top:0, flexDirection:'row'}}>
                    ...
                    <View style={{flex:1, flexDirection:'row', justifyContent:'flex-end', alignItems:'center', marginRight:10}}>
                        ...
                        <View style={{ justifyContent:'center', alignItems:'center' }}>
                            <SvgXml width='25' height='25' fill={this.props.themeColors.white} xml={SVGs.collectionUncolored}></SvgXml>
                        </View>
                    </View>
                </View>
                <View style={{position:'absolute', width:'100%', flexDirection:'row', bottom:30}}>
                    <View style={{flex:1, alignItems:'flex-end'}}>
                        <TouchableOpacity onPress={() => this.swiper.swipeLeft()} style={{padding:30, borderRadius:50, backgroundColor:this.props.themeColors.white, 
                            ...
                            shadowRadius: 8.84,
                            elevation: 2}}>
                            <SvgXml width='35' height='35' fill={this.props.themeColors.pink} xml={SVGs.tinderCancel}></SvgXml>
                        </TouchableOpacity>
                    </View>
                    <View style={{flex:.3}}/>
                    <View style={{flex:1, alignItems:'flex-start'}}>
                        <TouchableOpacity onPress={() => this.swiper.swipeRight()} style={{padding:30, borderRadius:50, backgroundColor:this.props.themeColors.white, 
                            shadowColor: this.props.themeColors.rowShadow,
                            shadowOffset: {
                            width: 0,
                            height: 0,
                            ...
                            shadowRadius: 8.84,
                            elevation: 2}}>
                            <SvgXml width='40' height='40' fill={this.props.themeColors.collectionMain} xml={SVGs.collectionUncolored}></SvgXml>
                        </TouchableOpacity>
                    </View>
                </View>
              </View>
...
export class EarnCollection extends Component {
    constructor(props) {
        super(props);
        this.page = 1;
    ...
    updateCoinView = (state) => {
        this.setState({coinAnimation:state}, () => setTimeout(() => {this.setState({coinAmount: this.state.coinAmount+50, coinAnimation: !state})}, 50))
    } 
        ...
        return (
            <View style={{backgroundColor:this.props.themeColors.screenBackground, justifyContent:'center', alignItems:'center', padding:8}}>           
                <Text style={{fontSize:25, fontFamily:FontFamilies.settingsRowContent, marginBottom:10, color:this.props.themeColors.headerTitle, textAlign:'center'}}>
                  Koleksiyona Al<Text style={{fontSize:27,  fontFamily: FontFamilies.settingsRowTitle, textTransform:'capitalize'}}> & Kazan</Text> 
                </Text>
                <Text style={{fontSize:15, paddingHorizontal:10, fontFamily:FontFamilies.historyTitle, color:this.props.themeColors.headerDesc, textAlign:'center'}}>
                    Gönderileri koleksiyona ekleyerek, koleksiyon başına <Text style={{color:this.props.themeColors.coinText, fontSize:17,  fontFamily: FontFamilies.settingsRowTitle, textTransform:'capitalize'}}> 50 Kredi</Text> kazanabilirsin.
                </Text>
            </View>
        );
    };
    ...
    //stackexchange User API url
        ...
        .then(res => {
            ...
            let data = cards.concat(res.data.results) //concate list with response
    ...
    render() {
        return (
            <View style={[styles.container,{flex:1, backgroundColor:this.props.themeColors.screenBackground}]}>
                {this.renderHeader()}
...
export default connect(mapStateToProps, mapDispatchToProps)(EarnCollection);
