...
import LikeRow from '../../../tabs/HistoryTab/RowTypes/LikeRow'
import {likeData} from '../../../tabs/HistoryTab/data'
...
class GetLikeScreen extends Component {
  ...
  renderHeader = () => (
    <View style={{}}>
        <View style={{ justifyContent:'center', alignItems:'center', padding:8, paddingTop:16}}>           
          <Text style={{fontSize:25, fontFamily:FontFamilies.settingsRowContent, marginBottom:10, color:this.props.themeColors.headerTitle, textAlign:'center'}}>
            Beğeni<Text style={{fontSize:27,  fontFamily: FontFamilies.settingsRowTitle, textTransform:'capitalize'}}> Al</Text> 
          </Text>
          <Text style={{fontSize:15, paddingHorizontal:10, fontFamily:FontFamilies.historyTitle, color:this.props.themeColors.headerDesc, textAlign:'center', lineHeight:23}}>
              İstediğin miktarı girerek seçtiğin gönderiye beğeni alabilirsin, beğeni başına <Text style={{color:this.props.themeColors.coinText, fontSize:17,  fontFamily: FontFamilies.settingsRowTitle, textTransform:'capitalize'}}> 30 Kredi</Text> ödersin.
          </Text>
        </View> 
    ...
    return (
        <SafeAreaView style={{backgroundColor:this.props.themeColors.screenBackground}}>
          <FlatList 
              ...
              style={{}}
              renderItem={({ item }) => <LikeRow item={item} isDone={item.done/item.total == 1} /> }
...
export default connect(mapStateToProps)(GetLikeScreen);
